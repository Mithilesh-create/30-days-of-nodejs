    ...
    const pipeline = [
      {
        $group: {
          ...
          totalProducts: { $sum: 1 }, // Count the total documents
          averagePrice: { $avg: '$price' }, // Calculate the average price
          highestQuantity: { $max: '$quantity' }, // Get the maximum quantity
        },
      },
    ];

    const results = await ProductWithCategory.aggregate(pipeline);
    return results[0]; // Return the first element (aggregated results)
  } catch (error) {
    throw error;
  }
};
